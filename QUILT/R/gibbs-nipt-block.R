...
for_testing_get_full_package_probabilities <- function(localH, fpp_stuff, maxEmissionMatrixDifference = 1e6) {
    ...
    package <- lapply(1:3, function(hap) {
        ...
        ## initialize
        out <- initialize_gibbs_forward_backward(H = localH, hap = hap, s = s, sampleReads = sampleReads, priorCurrent_m = priorCurrent_m, alphaMatCurrent_tc = alphaMatCurrent_tc, transMatRate_tc_H = transMatRate_tc_H, eMatRead_t = eMatRead_t, eMatGrid_t = eMatGrid_t, maxEmissionMatrixDifference = maxEmissionMatrixDifference)
...
plot_attempt_to_reblock_snps <- function(
    ...
    png(outname, height = 15, width = 20, res = 300, units = "in")
    par(mfrow = c(11, 1))
    par(oma = c(0, 0, 5, 0))    
