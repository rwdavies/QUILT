...
test_that("can avoid normalizing alphaHat throughout forward backwards, but recover correct gamma when asked, as well as dosages", {
        ...
        for(language in languages) {
            ...
            if (always_normalize) {
                if (language == "R") {
                    ...
                    Rcpp_alphaHat_t_always_norm <- alphaHat_t
                    Rcpp_dosage_always_norm <- dosage
                    Rcpp_gammaSmall_t_always_norm <- gammaSmall_t
                    Rcpp_c_always_norm <- c
                } else if (language == "Rcpp2") {
                    Rcpp2_alphaHat_t_always_norm <- alphaHat_t                    
                    Rcpp2_dosage_always_norm <- dosage
                    Rcpp2_gammaSmall_t_always_norm <- gammaSmall_t
                    Rcpp2_c_always_norm <- c
                }
            } else {
                if (language == "R") {
                    ...
                    Rcpp2_dosage_seldom_norm <- dosage
                    Rcpp2_gammaSmall_t_seldom_norm <- gammaSmall_t
                    Rcpp2_c_seldom_norm <- c                    
                }
            }
    ...
    if (!speed_test) {
        ...
        m <- (cbind(
            ...
            log(Rcpp2_c_always_norm),
            log(Rcpp2_c_seldom_norm)
        ))
        ## print(    colSums(m))
        ## print("-------")
        ## print(m)
    ...
    ##
    ##
    expect_equal(Rcpp_dosage_always_norm, Rcpp2_dosage_always_norm, tolerance = 1e-5) ## seems OK!
    x1 <- Rcpp_alphaHat_t_always_norm[, 10] / prod(Rcpp_c_always_norm[1:10])
    x2 <- Rcpp2_alphaHat_t_always_norm[, 10] / prod(Rcpp2_c_always_norm[1:10])
    ##expect_equal(Rcpp_dosage_always_norm, Rcpp_dosage_seldom_norm)
    ##expect_equal(Rcpp_dosage_always_norm, Rcpp2_dosage_seldom_norm)        
