...
## also do STITCH_again and STITCH_prepare_reference
cli_output_file <- "STITCH_again.R"
make_STITCH_cli(
    function_file = "STITCH/R/stitch-again.R",
    cli_output_file = cli_output_file,
    other_integer_params = "downsample_seed",
    other_logical_params = c("keep_generated_input", "rematch_SNPs", "gibbs_initialize_iteratively", "haploid_gibbs_equal_weighting"),
    other_character_params = c("sample_posfile", "phasefile"),
    function_name = "STITCH_again"
)
system(paste0("chmod +x ", cli_output_file))
cli_output_file <- "STITCH_prepare_reference.R"
make_STITCH_cli(
    function_file = "STITCH/R/stitch-prepare-reference.R",
    cli_output_file = cli_output_file,
    other_logical_params = "keep_generated_input",
    function_name = "STITCH_prepare_reference"
)
system(paste0("chmod +x ", cli_output_file))
...
make_STITCH_cli(
    ...
    cli_output_file = cli_output_file,
    other_logical_params = c("gibbs_initialize_iteratively", "haploid_gibbs_equal_weighting"),
    other_character_params = "phasefile"
)
system(paste0("chmod +x ", cli_output_file))

for(cli_output_file in c("STITCH.R", "STITCH_prepare_reference.R", "STITCH_again.R")) {
    message(paste0("test that ", cli_output_file, " produces help message"))
...
cli_output_file <- "STITCH.R" ## switch back to main
...
if (error_check == 1) {
    ...
    a <- length(grep("Build VCF from input", stderr))
    if (a != 1) {
        print(stdout)
        print(stderr)
        stop("failed on a")
    ...
    b <- length(grep("teration", stderr[-1]))
    if (b != 0) {
        print(stderr)
        stop("failed on b")        
    }
...
message("test that STITCH CLI parses GWS as NA correctly")
stdout_file <- tempfile()
stderr_file <- tempfile()
out <- system2(
    cli_output_file,
